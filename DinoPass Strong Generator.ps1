...
for ($i = 0; $i -lt $numberOfPasswords; $i++) {
    ...
    $response = Invoke-WebRequest -Uri "https://www.dinopass.com/password/strong" | Select-Object -ExpandProperty content
...
$passwords | Export-Csv -Path "C:\strongpasswords.csv" -NoTypeInformation
